...
def receiver_gen(team):
	for p in players.receiving().filter(receiving_rec_gte = 0).sort('receiving_rec'):
		if (str(p.guess_position) == 'TE' or str(p.guess_position) == '') and str(p.team) == team:
...
#['ATL', 'CAR', 'DET', 'GB', 'MIA', 'MIN', 'NO', 'OAK', 'PHI', 'PIT', 'TB']

for i in combine_data(teams):
	tight_ends.append({'A_Name':i[0], 'B_Team':i[1], 'C_Pos':i[2], 'D_Rec':i[3], 'E_Rec_Yds':i[4], 'F_Rec_Tds':i[5], 'G_Rush_Yds':i[6], 'H_Rush_Tds':i[7], 'I_Fum_Lost':i[8], 'J_FD_PTS':i[9]})

picks = pd.DataFrame(tight_ends)
np.savetxt('tight_ends_5_week_trend.txt', picks.sort(columns = 'J_FD_PTS', ascending = False), fmt='%r')
