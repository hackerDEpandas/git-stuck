...
#########################################################################
	def receiver_pts(self):
		self.total_pts = sum([self.instance_rushing.receiving_rec_pts(), self.instance_rushing.receiving_yds_pts(), self.instance_rushing.receiving_tds_pts(), self.instance_rushing.rushing_yds_pts(), self.instance_rushing.rushing_tds_pts(), self.instance_rushing.fumbles_lost_pts()])
		return round(self.total_pts,2)
#########################################################################
...
########################################################################
	def running_backs(self):
		...
		for p in self.data.rushing().filter(rushing_tds_gt = 0).limit(100):
...
########################################################################
	def wide_receivers(self):
		...
		for p in self.data.receiving().filter(receiving_rec_gt = 0).limit(120):
			self.instance_fanduel_pts = fanduel_pts(self.data, str(p), str(p.team))
...
#######################################################################
	def quarter_backs(self):
		...
		for p in self.data.passing().filter(passing_yds_gt = 0).limit(32):
			self.instance_fanduel_pts = fanduel_pts(self.data, str(p), str(p.team))
